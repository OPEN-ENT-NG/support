...
public abstract class ExportWorker extends AbstractVerticle {
    ...
    protected void run(Message<JsonObject> event) {
        JsonObject params = event.body().getJsonObject(Ticket.PARAMS);
        String action = event.body().getString(Ticket.ACTION);
        ...
        init(config())
                .compose(v -> getData(action, params))
                .compose(file -> exportData(null, user))
    ...
    protected abstract Future<ExportFile> getData(String action, JsonObject params);
        ...
        workspaceService.listRootDocuments(user)
                ...
                .onSuccess(files -> {
                    boolean folderExists = ((List<JsonObject>) files.getList())
                            .stream().anyMatch(res -> Objects.equals(res.getString(Ticket.NAME), folderName));
                    if (folderExists) {
